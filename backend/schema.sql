...
-- PRD §9 Self-Healing - real v1 remote dispatch for the 6 already-built remediation actions
-- (flush-dns/clean-temp/restart-service/clear-teams-cache/repair-vpn/collect-bsod-diagnostics,
-- see telemetry-server.mjs's own PRD §9 comment). An admin enqueues one here; the device discovers
-- it on its own next heartbeat poll (see
